...
import com.example.prodqapi.orderItem.OrderItemDTO;
import com.example.prodqapi.orderItem.OrderItemRepository;
import com.example.prodqapi.supplier.Supplier;
import com.example.prodqapi.supplier.SupplierRepository;
import com.example.prodqapi.supplier.SupplierPerformanceService;
import com.example.prodqapi.tool.Tool;
import com.example.prodqapi.tool.ToolRepository;
import com.example.prodqapi.invoiceItem.InvoiceItem;
import com.example.prodqapi.invoiceItem.InvoiceItemRepository;
import com.example.prodqapi.invoiceReconciliation.InvoiceReconciliation;
import com.example.prodqapi.invoiceReconciliation.InvoiceReconciliationRepository;
import com.example.prodqapi.invoiceReconciliation.InvoiceDiscrepancy;
...
public class OrderService {
    ...
    public Order addOrder(OrderDTO orderDTO) {
        ...
        notificationService.sendNotification(NotificationDescription.OrderAdded, Map.of("name", order.getName()));
    ...
    public void deleteOrder(Integer id) {
        ...
        if (orderOptional.isPresent()) {
            ...
            notificationService.sendNotification(NotificationDescription.OrderDeleted, Map.of("name", order.getName()));
        }
        ...
        if (orderOptional.isPresent()) {
            ...
            if (newStatus != null && !newStatus.equals(existingOrder.getStatus())) {
                ...
                if ("pending".equals(newStatus)) {
                    ...
                    notificationService.sendNotification(NotificationDescription.OrderOnTheWay, Map.of("name", existingOrder.getName(), "date", existingOrder.getDate()));
                    ...
                    deleteQuantityInTransport(orderDTO.getId());
                    notificationService.sendNotification(NotificationDescription.OrderDelivered, Map.of("name", existingOrder.getName(), "date", existingOrder.getDate()));
                }
            }
    ...
    public void partialDelivery(Integer orderId, List<OrderItem> updatedItems) {
        ...
        if (orderOptional.isPresent()) {
            ...
            if (allItemsFullyReceived) {
                ...
                notificationService.sendNotification(NotificationDescription.OrderAllItemsReceived, Map.of("name", order.getName()));
            } else {
                order.setStatus("partially_delivered");
                // Do NOT set externalQuantityUpdated = true (allow future deliveries)
                notificationService.sendNotification(NotificationDescription.OrderPartiallyDelivered, Map.of("name", order.getName()));
            }
    ...
    public void updateQualityRating(Integer orderId, Integer rating, String notes) {
        ...
        if (orderOptional.isPresent()) {
            ...
            notificationService.sendNotification(NotificationDescription.OrderQualityRated, Map.of("name", order.getName(), "rating", String.valueOf(rating)));
        } else {
    ...
    public void markInvoicePending(Integer orderId) {
        ...
        if (orderOptional.isPresent()) {
            ...
            if ("goods_received".equals(order.getStatus())) {
                order.setStatus("invoice_pending");
                notificationService.sendNotification(NotificationDescription.OrderInvoicePending, Map.of("name", order.getName()));
                orderRepository.save(order);
            }
        }
    }
    ...
    public void markInvoiceReceived(Integer orderId) {
        ...
        if (orderOptional.isPresent()) {
            ...
            if ("invoice_pending".equals(order.getStatus()) && order.getInvoiceFileName() != null) {
                ...
                notificationService.sendNotification(NotificationDescription.OrderInvoiceReceived, Map.of("name", order.getName()));
                orderRepository.save(order);
            }
        }
    }
    ...
    public void closeOrder(Integer orderId) {
        ...
        if (orderOptional.isPresent()) {
            ...
            if ("invoice_received".equals(order.getStatus())) {
                ...
                notificationService.sendNotification(NotificationDescription.OrderClosed, Map.of("name", order.getName()));
                orderRepository.save(order);
            }
        }
    }
    ...
    public void closeOrderShort(Integer orderId, String reason) {
        ...
        if (orderOptional.isPresent()) {
            ...
            notificationService.sendNotification(NotificationDescription.OrderMarkedForIncompleteClose, Map.of("name", order.getName()));
            orderRepository.save(order);
        } else {
            throw new IllegalArgumentException("Order not found with ID: " + orderId);
        }
    ...
    public void updateItemPrice(Integer orderId, Integer orderItemId, BigDecimal newPrice) {
        ...
        if (orderOptional.isPresent()) {
            ...
            notificationService.sendNotification(NotificationDescription.OrderItemPriceUpdated, Map.of("name", order.getName()));
        }
    }
    ...
    public void saveInvoiceItems(Integer orderId, InvoiceItemsDTO dto) {
        ...
        notificationService.sendNotification(NotificationDescription.InvoiceItemsEntered, Map.of("name", order.getName()));
    }
    ...
    public void approveInvoiceDiscrepancies(Integer orderId, String justification) {
        ...
        notificationService.sendNotification(NotificationDescription.InvoiceDiscrepanciesApproved, Map.of("name", order.getName()));
    }
